...
public final class ConnectedBlockSemantics {
    ...
    public static TableState eastWest(TableState current, boolean east, boolean west) {
        if (current.axis() == Axis.Z && current.position() != 0) return current;
        if (east && west) return new TableState(Axis.X, 2);
        if (east) return new TableState(Axis.X, 1);
        if (west) return new TableState(Axis.X, 3);
        return new TableState(current.axis(), 0);
    }
    public static TableState northSouth(TableState current, boolean south, boolean north) {
        if (current.axis() == Axis.X && current.position() != 0) return current;
        if (south && north) return new TableState(Axis.Z, 2);
        if (south) return new TableState(Axis.Z, 1);
        if (north) return new TableState(Axis.Z, 3);
        return new TableState(current.axis(), 0);
    }
}
