...
final class BlockSettings {
    ...
    static JsonObject create(String blockId, boolean hasItem) {
        ...
        result.addProperty("push_reaction", blockId.equals("pressing_tub") ? "BLOCK" : sturdy || blockId.equals("tap") ? "NORMAL" : "DESTROY");
        ...
        else if (!wild && !crop && !INCENSE.contains(blockId)) tags.add("minecraft:mineable/axe");
    ...
    private static void combustible(JsonObject target, int color, String instrument) {
        target.addProperty("map_color", color); target.addProperty("instrument", instrument); target.addProperty("burnable", true);
        target.addProperty("burn_chance", 5); target.addProperty("fire_spread_chance", 20);
    }
}
