...
public final class ItemMigrationStage {
    ...
    private JsonObject shakerModel(){return obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("gui","fixed"),"model",model("shaker"))),"fallback",obj("type","minecraft:condition","property","minecraft:using_item","on_true",obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("firstperson_righthand"),"model",useCycle(true)),obj("when",arr("firstperson_lefthand"),"model",useCycle(false)),obj("when",arr("thirdperson_lefthand","thirdperson_righthand"),"model",model("shaker_3d"))),"fallback",model("shaker_3d")),"on_false",model("shaker_3d")));}
    /** 第一人称：bow 动画会给物品附加 ItemInHandRenderer case 5 的持握变换 C（yRot∓45°、scale 1.2 等），故每帧矩阵取 U=D⁻¹·C⁻¹·D·U0（D=firstperson display、U0=0.0.1 的 -15°+0.15·sin(1.5t) 摆动），净效果与 v0.0.1 逐位一致（bow 无 customArmTransform，手臂正常）。第三人称 bow 姿势双臂前伸 yRot≈0（原模组方向）、基准 -90°，WHACK 挥臂在其上叠加 ±42° 摆动。 */
    private JsonObject useCycle(boolean right){JsonArray entries=new JsonArray();double period=Math.PI*2/1.5;double[] d=mul4(mul4(trans(0,2.75,0),scal(0.5)),trans(-0.5,-0.5,-0.5));double[] c=bowHand(right),invD=inv4(d),invC=inv4(c);for(int i=0;i<16;i++){double cycle=period*i/16,wave=Math.sin(-cycle*1.5),ty=-wave*.15;double[] u0=mul4(trans(0,ty,0),rotX(-15));double[] u=mul4(mul4(mul4(invD,invC),d),u0);entries.add(obj("threshold",round(cycle,6),"model",obj("type","minecraft:model","path",NAMESPACE+":item/shaker_3d","transformation",arr16(u))));}return obj("type","minecraft:range_dispatch","property","use_cycle","source",round(period,6),"entries",entries,"fallback",entries.get(0).getAsJsonObject().get("model").deepCopy());}
