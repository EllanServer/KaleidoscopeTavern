...
public final class ItemMigrationStage {
    ...
    private JsonObject shakerModel(){return obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("gui","fixed"),"model",model("shaker"))),"fallback",obj("type","minecraft:condition","property","minecraft:using_item","on_true",obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("firstperson_righthand"),"model",useCycle(true)),obj("when",arr("firstperson_lefthand"),"model",useCycle(false)),obj("when",arr("thirdperson_righthand"),"model",useCycleTP(true)),obj("when",arr("thirdperson_lefthand"),"model",useCycleTP(false))),"fallback",model("shaker_3d")),"on_false",model("shaker_3d")));}
    ...
    /** 第三人称 use_cycle：右手 ΔxRot = -45°·sin(1.5t)、zRot -9°；左手镜像反相（源 SHAKING 的 else 分支 +45°·sin、+9°），左右手各 16 帧、周期 2π/1.5。 */
