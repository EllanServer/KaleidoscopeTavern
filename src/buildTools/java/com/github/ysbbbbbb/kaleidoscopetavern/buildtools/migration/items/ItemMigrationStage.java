...
public final class ItemMigrationStage {
    ...
    private JsonObject buildItems(List<String> itemIds, Set<String> blockIds, Set<String> furnitureIds,
        ...
        for (String id : itemIds) {
            ...
            if (id.equals("shaker")) { components(data).addProperty("minecraft:max_stack_size",1);
                configureConsumable(config,obj("consume_seconds",3600.0,"animation","bow","has_consume_particles",false));
    ...
    private JsonObject shakerModel(){return obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("gui","fixed"),"model",model("shaker"))),"fallback",obj("type","minecraft:condition","property","minecraft:using_item","on_true",obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("firstperson_righthand"),"model",useCycle(true)),obj("when",arr("firstperson_lefthand"),"model",useCycle(false)),obj("when",arr("thirdperson_lefthand","thirdperson_righthand"),"model",useCycleTP())),"fallback",model("shaker_3d")),"on_false",model("shaker_3d")));}
    /** 第一人称：bow 动画会给物品附加 ItemInHandRenderer case 5 的持握变换 C（yRot∓45°、scale 1.2 等），故每帧矩阵取 U=D⁻¹·C⁻¹·D·U0（D=firstperson display、U0=0.0.1 的 -15°+0.15·sin(1.5t) 摆动），净效果与 v0.0.1 逐位一致（bow 无 customArmTransform，手臂正常）。range_dispatch 用 26.2 原版格式（property 带前缀、period/scale 字段——v0.0.1 的 source 字段无效导致周期恒 1、动画静止）。第三人称：完全由客户端 use_cycle 循环驱动（无服务端挥动包、无 WHACK 横扫），雪克杯在手里按原模组 SHAKING 公式摆动（ΔxRot = -45°·sin(1.5t)、zRot 恒定 -9°），手臂保持 bow 前伸姿势。 */
    /** 第三人称 use_cycle：16 帧、source 2π/1.5（原模组速度），每帧 = R_X(-45°·sin(1.5t))·R_Z(-9°)——复刻原模组 SHAKING 的摆动方向与幅度。 */
    private JsonObject useCycleTP(){JsonArray entries=new JsonArray();double period=Math.PI*2/1.5;for(int i=0;i<16;i++){double cycle=period*i/16,angle=-45*Math.sin(1.5*cycle);entries.add(obj("threshold",round(cycle,6),"model",obj("type","minecraft:model","path",NAMESPACE+":item/shaker_3d","transformation",arr16(mul4(rotX(angle),rotZ(-9))))));}return obj("type","minecraft:range_dispatch","property","minecraft:use_cycle","period",round(period,6),"scale",1.0,"entries",entries,"fallback",entries.get(0).getAsJsonObject().get("model").deepCopy());}
    private JsonObject useCycle(boolean right){JsonArray entries=new JsonArray();double period=Math.PI*2/1.5;double[] d=trans(-0.5,-0.5,-0.5);double[] c=bowHand(right),invD=trans(0.5,0.5,0.5),invC=inv4(c);for(int i=0;i<16;i++){double cycle=period*i/16,wave=Math.sin(-cycle*1.5),ty=-wave*.15;double[] u0=mul4(trans(0,ty,0),rotX(-15));double[] u=mul4(mul4(mul4(invD,invC),d),u0);entries.add(obj("threshold",round(cycle,6),"model",obj("type","minecraft:model","path",NAMESPACE+":item/shaker_3d","transformation",arr16(u))));}return obj("type","minecraft:range_dispatch","property","minecraft:use_cycle","period",round(period,6),"scale",1.0,"entries",entries,"fallback",entries.get(0).getAsJsonObject().get("model").deepCopy());}
    /** ItemInHandRenderer 第一人称 bow（case 5）静态持握变换（拉弓 20 tick 充满后）：R_Y(-45°·dir)·S(1.2)·T(0,0,0.04)·R_Z(-9.785°·dir)·R_Y(35.3°·dir)·R_X(-13.935°)·T(-0.2785682·dir,0.18344387,0.15731531)。 */
    private static double[] bowHand(boolean right){double dir=right?1:-1;return mul4(mul4(mul4(mul4(mul4(mul4(rotY(-45*dir),scal(1.2)),trans(0,0,0.04)),rotZ(-9.785*dir)),rotY(35.3*dir)),rotX(-13.935)),trans(-0.2785682*dir,0.18344387,0.15731531));}
    private static double[] ident(){double[] m=new double[16];for(int i=0;i<16;i++)m[i]=i%5==0?1:0;return m;}
    private static double[] trans(double x,double y,double z){double[] m=ident();m[3]=x;m[7]=y;m[11]=z;return m;}
    private static double[] scal(double s){double[] m=ident();m[0]=s;m[5]=s;m[10]=s;return m;}
    ...
    private static double[] rotY(double deg){double r=Math.toRadians(deg),c=Math.cos(r),s=Math.sin(r);double[] m=ident();m[0]=c;m[2]=s;m[8]=-s;m[10]=c;return m;}
    ...
    private static double[] inv4(double[] m){double[] a=m.clone();double[] r=ident();for(int col=0;col<4;col++){int p=col;for(int i=col+1;i<4;i++)if(Math.abs(a[i*4+col])>Math.abs(a[p*4+col]))p=i;if(p!=col){for(int j=0;j<4;j++){double t=a[col*4+j];a[col*4+j]=a[p*4+j];a[p*4+j]=t;t=r[col*4+j];r[col*4+j]=r[p*4+j];r[p*4+j]=t;}}double dd=a[col*4+col];for(int j=0;j<4;j++){a[col*4+j]/=dd;r[col*4+j]/=dd;}for(int i=0;i<4;i++){if(i==col)continue;double f=a[i*4+col];if(f==0)continue;for(int j=0;j<4;j++){a[i*4+j]-=f*a[col*4+j];r[i*4+j]-=f*r[col*4+j];}}}return r;}
