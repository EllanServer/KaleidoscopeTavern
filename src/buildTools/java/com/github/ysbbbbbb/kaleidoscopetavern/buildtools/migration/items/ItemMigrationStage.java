...
public final class ItemMigrationStage {
    ...
    private JsonObject shakerModel(){return obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("gui","fixed"),"model",model("shaker"))),"fallback",obj("type","minecraft:condition","property","minecraft:using_item","on_true",obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("firstperson_righthand"),"model",useCycle(true,true)),obj("when",arr("firstperson_lefthand"),"model",useCycle(true,false)),obj("when",arr("thirdperson_lefthand","thirdperson_righthand"),"model",model("shaker_3d"))),"fallback",model("shaker_3d")),"on_false",model("shaker_3d")));}
    /** 第一人称：trident 动画会附加三叉戟持握变换 C（ItemInHandRenderer case 6）。0.0.1 版本（animation none）没有 C，故每帧矩阵取 U=D⁻¹·C⁻¹·D·U0，其中 U0 为 0.0.1 的 -15°+上下摆动、D 为 firstperson display，使第一人称净效果与 0.0.1 完全一致。 */
    private JsonObject useCycle(boolean first,boolean right){JsonArray entries=new JsonArray();double period=Math.PI*2/1.5;double[] d=mul4(mul4(trans(0,2.75,0),scal(0.5)),trans(-0.5,-0.5,-0.5));double[] c=tridentHand(right),invD=inv4(d),invC=inv4(c);for(int i=0;i<16;i++){double cycle=period*i/16,wave=Math.sin(-cycle*1.5),ty=-wave*.15;double[] u0=mul4(trans(0,ty,0),rotX(-15));double[] u=mul4(mul4(mul4(invD,invC),d),u0);entries.add(obj("threshold",round(cycle,6),"model",obj("type","minecraft:model","path",NAMESPACE+":item/shaker_3d","transformation",arr16(u))));}return obj("type","minecraft:range_dispatch","property","use_cycle","source",round(period,6),"entries",entries,"fallback",entries.get(0).getAsJsonObject().get("model").deepCopy());}
    /** ItemInHandRenderer 第一人称 trident 用例（case 6）的静态变换。 */
    private static double[] tridentHand(boolean right){double dir=right?1:-1;double[] m=mul4(mul4(mul4(mul4(mul4(mul4(trans(-0.5*dir,0.7,0.1),rotX(-55)),rotY(35.3*dir)),rotZ(-9.785*dir)),trans(0,0,0.2)),scal(1.2)),rotY(-45*dir));return m;}
    ...
    private static double[] inv4(double[] m){double[] a=m.clone();double[] r=ident();for(int col=0;col<4;col++){int p=col;for(int i=col+1;i<4;i++)if(Math.abs(a[i*4+col])>Math.abs(a[p*4+col]))p=i;if(p!=col){for(int j=0;j<4;j++){double t=a[col*4+j];a[col*4+j]=a[p*4+j];a[p*4+j]=t;t=r[col*4+j];r[col*4+j]=r[p*4+j];r[p*4+j]=t;}}double d=a[col*4+col];for(int j=0;j<4;j++){a[col*4+j]/=d;r[col*4+j]/=d;}for(int i=0;i<4;i++){if(i==col)continue;double f=a[i*4+col];if(f==0)continue;for(int j=0;j<4;j++){a[i*4+j]-=f*a[col*4+j];r[i*4+j]-=f*r[col*4+j];}}}return r;}
