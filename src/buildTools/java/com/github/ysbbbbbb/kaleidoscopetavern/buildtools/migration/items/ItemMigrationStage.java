...
public final class ItemMigrationStage {
    ...
    /** 第三人称动画原理 = 望远镜（spyglass）动画：26.2 的 setupAttackAnimation 对任何姿势都叠加 WHACK 挥动，因此使用中手臂保持 spyglass 举臂姿势（-110°，接近原模组 -112.5°），服务端每 4 tick 的 swingHand 让 WHACK 在其上挥手（±42°），完全由客户端播放；左右手由客户端 getArmPose 各自判定。第一人称：spyglass 无 case 变换、无 customArmTransform、不变焦，use_cycle 16 帧 -15°+0.15·sin(1.5t) 与 v0.0.1 逐位一致（range_dispatch 用 26.2 period/scale 格式）。 */
