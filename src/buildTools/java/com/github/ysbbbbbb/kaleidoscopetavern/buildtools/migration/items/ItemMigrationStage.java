...
public final class ItemMigrationStage {
    ...
    private JsonObject buildItems(List<String> itemIds, Set<String> blockIds, Set<String> furnitureIds,
        ...
        for (String id : itemIds) {
            ...
            if (id.equals("shaker")) { components(data).addProperty("minecraft:max_stack_size",1);
                configureConsumable(config,obj("consume_seconds",3600.0,"animation","bow","has_consume_particles",false)); config.add("model", shakerModel()); }
    ...
    private JsonObject shakerModel(){return obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("gui","fixed"),"model",model("shaker"))),"fallback",obj("type","minecraft:condition","property","minecraft:using_item","on_true",obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("firstperson_lefthand","firstperson_righthand"),"model",useCycle(true)),obj("when",arr("thirdperson_righthand"),"model",useCycle(false,true)),obj("when",arr("thirdperson_lefthand"),"model",useCycle(false,false))),"fallback",model("shaker_3d")),"on_false",model("shaker_3d")));}
    private JsonObject useCycle(boolean first){return useCycle(first,true);} private JsonObject useCycle(boolean first,boolean right){JsonArray entries=new JsonArray();double period=Math.PI*2/1.5;for(int i=0;i<16;i++){double cycle=period*i/16,wave=Math.sin(-cycle*1.5);JsonArray t=first?transform(-15,-wave*.15):thirdPersonTransform(right,wave);entries.add(obj("threshold",round(cycle,6),"model",obj("type","minecraft:model","path",NAMESPACE+":item/shaker_3d","transformation",t)));}return obj("type","minecraft:range_dispatch","property","use_cycle","source",round(period,6),"entries",entries,"fallback",entries.get(0).getAsJsonObject().get("model").deepCopy());}
    /** 第三人称：consumable 弓动画让手臂笔直举过头顶（ArmPose.BOW，xRot≈-π），逐帧把雪克杯从弓手位摆到原模组 ShakerAnimation.SHAKING 的手臂手位并同步俯仰（φ=1.375π∓0.25π·wave、zRot=∓0.05π）。26.2 客户端在 display 变换（含 0.5 缩放）之后应用 transformation，平移会被 0.5 折半，故 12px 手臂≈0.9 方块在此写成 1.8。 */
    private static JsonArray thirdPersonTransform(boolean right,double wave){double phi=1.375*Math.PI+(right?1:-1)*.25*Math.PI*wave;return heldPoseMatrix(phi,right?-9:9);}
    private static JsonArray heldPoseMatrix(double phi,double zDeg){double armX=phi+Math.PI,cx=Math.cos(armX),sx=Math.sin(armX),zr=Math.toRadians(zDeg),cz=Math.cos(zr),sz=Math.sin(zr),r00=cz,r01=-sz*cx,r02=sz*sx,r10=sz,r11=cz*cx,r12=-cz*sx,r21=sx,r22=cx,cosPhi=Math.cos(phi),sinPhi=Math.sin(phi),arcY=-1.8*(1+cosPhi),arcZ=-1.8*sinPhi,tx=.5-.5*(r00+r01+r02),ty=.5+arcY-.5*(r10+r11+r12),tz=.5+arcZ-.5*(r21+r22);return arr(round(r00,8),round(r01,8),round(r02,8),round(tx,8),round(r10,8),round(r11,8),round(r12,8),round(ty,8),0.0,round(r21,8),round(r22,8),round(tz,8),0.0,0.0,0.0,1.0);}
