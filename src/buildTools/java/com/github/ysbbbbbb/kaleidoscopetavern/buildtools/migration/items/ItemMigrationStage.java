...
public final class ItemMigrationStage {
    ...
    private JsonObject shakerModel(){return obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("gui","fixed"),"model",model("shaker"))),"fallback",obj("type","minecraft:condition","property","minecraft:using_item","on_true",obj("type","minecraft:select","property","display_context","cases",arr(obj("when",arr("firstperson_lefthand","firstperson_righthand"),"model",useCycle(true)),obj("when",arr("thirdperson_lefthand","thirdperson_righthand"),"model",useCycle(false))),"fallback",model("shaker_3d")),"on_false",model("shaker_3d")));}
    private JsonObject useCycle(boolean first){JsonArray entries=new JsonArray();double period=Math.PI*2/1.5;for(int i=0;i<16;i++){double cycle=period*i/16,wave=Math.sin(-cycle*1.5),rot=first?-15:Math.toDegrees(wave*.25),ty=first?-wave*.15:0;entries.add(obj("threshold",round(cycle,6),"model",obj("type","minecraft:model","path",NAMESPACE+":item/shaker_3d","transformation",transform(rot,ty))));}return obj("type","minecraft:range_dispatch","property","use_cycle","source",round(period,6),"entries",entries,"fallback",entries.get(0).getAsJsonObject().get("model").deepCopy());}
