...
public final class ItemMigrationStage {
    ...
    private JsonObject buildItems(List<String> itemIds, Set<String> blockIds, Set<String> furnitureIds,
        ...
        for (String id : itemIds) {
            ...
            if (id.equals("shaker")) { components(data).addProperty("minecraft:max_stack_size",1);
                configureConsumable(config,obj("consume_seconds",3600.0,"animation","none","has_consume_particles",false)); config.add("model", shakerModel()); }
    ...
    /** 第三人称逐帧复刻原模组 ShakerAnimation.SHAKING 手臂姿态：xRot=1.375π∓0.25π·wave、zRot=∓0.05π，并把雪克杯从自然下垂的手位移到抬臂后的手位。26.2 客户端在 display 变换（含 0.5 缩放）之后应用 transformation，平移会被 0.5 折半，故臂长 12px≈0.9 方块在此写成 1.8。 */
    ...
    private static JsonArray heldPoseMatrix(double phi,double zDeg){double cx=Math.cos(phi),sx=Math.sin(phi),zr=Math.toRadians(zDeg),cz=Math.cos(zr),sz=Math.sin(zr),r00=cz,r01=-sz*cx,r02=sz*sx,r10=sz,r11=cz*cx,r12=-cz*sx,r21=sx,r22=cx,raiseY=1.8*(1-cx),raiseZ=-1.8*sx,tx=.5-.5*(r00+r01+r02),ty=.5+raiseY-.5*(r10+r11+r12),tz=.5+raiseZ-.5*(r21+r22);return arr(round(r00,8),round(r01,8),round(r02,8),round(tx,8),round(r10,8),round(r11,8),round(r12,8),round(ty,8),0.0,round(r21,8),round(r22,8),round(tz,8),0.0,0.0,0.0,1.0);}
