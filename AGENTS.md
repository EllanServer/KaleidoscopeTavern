...
- 遵循 CraftEngine Wiki 对物品、方块和家具的区分。只有需要网格状态、随机刻、存活规则或世界生成的
  39 个公开/结构内容使用 CE 自定义方块；另有 15 个私有葡萄阶段方块供 CustomCrops 切换，总计 54 个
  CE 方块 id。桌椅、设备、酒瓶、灯具和展示柜等 120 个内容使用 CE 家具。八种熏香、水龙头、酒窖柜、
  倾斜酒架、圆形酒架与酒瓶托架属于 CE 方块，由自定义方块行为保存开关、储物与红石状态。
...
- 迁移与校验会生成/检查 157 个公共物品、54 个方块 id、120 个家具、554 个私有渲染物品、114 个配方
