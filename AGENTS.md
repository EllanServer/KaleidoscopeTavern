...
- 遵循 CraftEngine Wiki 对物品、方块和家具的区分。需要真实网格状态、邻居更新、随机刻、存活规则或
  世界生成的内容使用 CE 自定义方块；静态陈设和实体展示设备使用 CE 家具。当前生成 60 个 CE 方块 id，
...
- 十六色沙发共用一个私有 `kaleidoscope_tavern:_internal/sofa` CE 方块：16 个公共物品通过 `dyed_color`
  ...
  方块 id 暂保留为每个 4 状态的迁移别名；Java 只负责 CE 无法表达的六状态拓扑与一次性旧区块迁移。
...
- 迁移与校验会生成/检查 157 个公共物品、60 个方块 id、137 个家具定义（含一次性迁移定义）、503 个私有渲染物品、114 个配方
