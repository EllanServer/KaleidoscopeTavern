...
- 沙发、桌子、吧台与两种吧台柜改为真实 CE 方块：物品放置、朝向、含水、载体碰撞、模型、掉落和沙发座位均由 CE
  原生行为或生成配置负责。Java 只保留 CE 26.7.4 配置无法表达的六状态转角连接、桌子世界坐标轴连接，
  放置时从当前位置读取初始含水状态这一 API 缺口，以及双展示槽、异形酒瓶和精确展示变换等 CE 原生展示槽无法完整表达的源玩法语义。
...
- 家具业务状态使用家具元实体数据；CE 方块业务状态使用方块实体 NBT。静态碰撞、座位、放置、朝向、
